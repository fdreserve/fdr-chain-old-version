...
#include "ui_zfdreservecontroldialog.h"
...
std::list<std::string> ZFDReserveControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZFDReserveControlDialog::listMints;

ZFDReserveControlDialog::ZFDReserveControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZFDReserveControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZFDReserveControlDialog::~ZFDReserveControlDialog()
{
    delete ui;
}

void ZFDReserveControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZFDReserveControlDialog::updateList()
{
...
void ZFDReserveControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZFDReserveControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZFDReserve_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZFDReserveControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZFDReserveControlDialog::GetSelectedMints()
{
...
void ZFDReserveControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
