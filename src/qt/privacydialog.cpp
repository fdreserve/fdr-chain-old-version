...
#include "zfdreservecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zFDR ought to be enough for anybody." - Bill Gates, 2017
    ui->zFDRpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzFDRSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzFDR->setEnabled(false);
        ui->pushButtonMintzFDR->setToolTip(tr("zFDR is currently disabled due to maintenance."));

        ui->pushButtonSpendzFDR->setEnabled(false);
        ui->pushButtonSpendzFDR->setToolTip(tr("zFDR is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zFDRpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzFDR_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFDR is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zFDR...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zFDR in ") +
...
void PrivacyDialog::on_pushButtonSpendzFDR_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFDR is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zFDR
        sendzFDR();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zFDR
    sendzFDR();
}

void PrivacyDialog::on_pushButtonZFDReserveControl_clicked()
{
    ZFDReserveControlDialog* zFDRControl = new ZFDReserveControlDialog(this);
    zFDRControl->setModel(walletModel);
    zFDRControl->exec();
}

void PrivacyDialog::setZFDReserveControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzFDRSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzFDR()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid FDReserve Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zFDRpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zFDR is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " FDRESERVE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zFDR</b>";
    ...
    // use mints from zFDR selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZFDReserveControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZFDReserveControlDialog::GetSelectedMints();
    }
    ...
    // Clear zfdreserve selector in case it was used
    ZFDReserveControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zFDR Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " FDReserve, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zFDR Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zFDR </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zFDR "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zFDR "));
    ui->labelzFDRAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzFDRSyncStatus->setVisible(fShow);
}
